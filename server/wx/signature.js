...
var BufferHelp = Meteor.npmRequire('bufferhelper');
...
wugg = {
    ...
    'getSignature': function (config, url, cb) {
        console.log('start getSignature');
        // 判断内存中是否有缓存
        if (!wugg.cache || !wugg.cache.ticket) {
            console.log('readCache');
            wugg.readFile(process.env.PWD+'/server/wx/cache.json', function (str) {
                if (str) {
                    console.log(str);
                    wugg.cache = JSON.parse(str);
                }
                wugg.tryGetSignature(config, url, cb);
            });
        }
        else {
            wugg.tryGetSignature(config, url, cb);
    ...
    'checkSignature': function (config) {
    ...
    'getToken': function (config, cb) {
        ...
        console.log("getToken>>> tokenUrl:"+tokenUrl);
    ...
    'getNewTicket': function (token, cb) {
        console.log("----------getNewTicket----------------:"+token);
    ...
    'tryGetSignature': function (config, u, cb) {
        // 判断cache 是否过期
        if (!wugg.cache.ticket || (new Date().getTime() - wugg.cache.time) > 7000000) {
            async.waterfall([function (cb) {
                console.log('start getNew Ticket', wugg.cache);
                wugg.getToken(config, cb);
            }, function (token, cb) {
                wugg.getNewTicket(token, cb);
                ...
                if (error) {
                    ...
                    // 文件保存
                    wugg.writeFile(process.env.PWD+'/server/wx/cache.json', JSON.stringify(wugg.cache));
                    console.log(result);

                    var timestamp = wugg.getTimesTamp();
                    var noncestr = wugg.getNonceStr();
